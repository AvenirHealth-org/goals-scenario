...
from goals_sa.cli import RunConfig, _load_config, app
...
def test_cli_scenarios_calls_generate_scenarios(tmp_path):
    ...
    with patch("goals_sa.cli.generate_scenarios") as mock_gen:
...
def test_cli_scenarios_handles_errors():
    with patch("goals_sa.cli.generate_scenarios", side_effect=RuntimeError("something went wrong")):
...
def test_cli_scenarios_handles_error_without_message():
    with patch("goals_sa.cli.generate_scenarios", side_effect=NotImplementedError()):
        result = runner.invoke(app, ["scenarios", "--dest-path", "/x/out.csv"])
